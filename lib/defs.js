const
  ...
  NATIVE_TYPES = {
    ...
    'DATE': {
      ...
      convert: val => {
        if (!val) return 'NULL'
        if (typeof val === 'string') val = moment(parseInt(val)).format('YYYY-MM-DD')
    ...
    'DATETIME': {
      ...
      convert: val => {
        if (!val) return 'NULL'
        if (typeof val === 'string') val = moment(parseInt(val)).format('YYYY-MM-DD HH:mm:ss')
      ...
      fields: Object.keys(collections[name])
        ...
        .map(fieldName => {
          ...
          return {
            ...
            primary: ~['id', '_id'].indexOf(fieldName) ? true : false
          }
        })
    }
  })
