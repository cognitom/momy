...
export default class MySQL {
   ...
   */
  readTimestamp () {
    const q = `SELECT timestamp FROM mongo_to_mysql WHERE service = '${this.dbName}'`
    return this.query(q)
   ...
   */
  updateTimestamp (ts) {
    const q = `UPDATE mongo_to_mysql SET timestamp = ${ts} WHERE service = '${this.dbName}';`
    this.getConnection()
      .query(q)
  }
  ...
  getConnection () {
    ...
    con.connect(function (err) {
      if (err) logger.error(`SQL CONNECT ERROR: ${err}`)
    })
    con.on('close', () => logger.info('SQL CONNECTION CLOSED.'))
    con.on('error', err => logger.error(`SQL CONNECTION ERROR: ${err}`))
