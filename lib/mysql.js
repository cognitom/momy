...
class MySQL {
  ...
  constructor (url) {
    ...
    this.query(ddl)
      .then(() => this.query(log))
    if (!fs.existsSync('logs/query.log')) {
      fs.writeFileSync('logs/query.log', '')
    }
  }
  ...
  update (def, id, item, unsetItems, callback) {
    ...
    const promise = this.query(sql)
      ...
      .catch(err => {
        util.log(sql)
        fs.appendFileSync('logs/error.log', `${new Date()}: MySQL.update -> ${err} \n`)
        throw err
      })
  ...
  remove (def, id, callback) {
    ...
    const promise = this.query(sql)
      .catch(err => {
        util.log(sql)
        fs.appendFileSync('logs/error.log', `${new Date()}: MySQL.remove -> ${err} \n`)
        throw err
      })
  ...
  removeRelation (tableName, id, callback) {
    ...
    const promise = this.query(sql, [id])
      .catch(err => {
        util.log(sql)
        fs.appendFileSync('logs/error.log', `${new Date()}: MySQL.removeRelation -> ${err} \n`)
        throw err
      })
  ...
  optimize(tableName) {
    ...
    return this.query(sql, [])
      .catch(err => {
        util.log(sql)
        fs.appendFileSync('logs/error.log', `${new Date()}: MySQL.optimize -> ${err} \n`)
        throw err
      })
  }
  ...
  readTimestamp () {
    ...
    return this.query(q)
      ...
      .catch(err => {
        util.log(q)
        fs.appendFileSync('logs/error.log', `${new Date()}: MySQL.readTimestamp -> ${err} \n`)
        throw err
      })
  }
  ...
  getConnection () {
    ...
    con.connect(function (err) {
      if (err) {
        fs.appendFileSync('logs/error.log', `${new Date()}: MySQL.getConnection.onConnect -> ${err} \n`)
    ...
    con.on('error', err => {
      fs.appendFileSync('logs/error.log', `${new Date()}: MySQL.getConnection.onError -> ${err} \n`)
