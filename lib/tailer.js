...
class Tailer {
  ...
  constructor (config) {
    ...
    this.defs   = createDefs(config.collections, this.dbName, config.prefix, config.snake_case)
    this.lastTs = 0
  ...
  process (log) {
    ...
    switch (log.op) {
      ...
      case 'u':
        util.log(`Update a record in ${ def.ns } (_id=${ log.o2._id })`)
        return this.mysql.update(def, log.o2._id, log.o.$set, log.o.$unset)
      case 'd':
        util.log(`Delete a record in ${ def.ns } (_id=${ log.o._id })`)
