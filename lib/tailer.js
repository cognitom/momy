...
class Tailer {
  ...
  start (forever) {
    ...
    this.mysql.readTimestamp()
      ...
      .catch(err => {
        fs.appendFileSync('logs/error.log', `${new Date()}: Tailer.start -> ${err} \n`)
  ...
  importAndStart (forever) {
    ...
    this.mysql.bulkDropTable(this.defs)
      ...
      .then(() => forever ? this.tailForever() : this.tail())
      .catch(err => {
        fs.appendFileSync('logs/error.log', `${new Date()}: Tailer.importAndStart -> ${err} \n`)
        return this.stop(err)
      })
  }
    ...
    return new Promise((resolve, reject) => {
      ...
      const chainPromise = () => {
        promise = promise
          ...
          .catch(err => {
            fs.appendFileSync('logs/error.log', `${new Date()}: Tailer.tailForever -> ${err} \n`)
            return reject(err)
          })
          .then(chainPromise)
      }
      ...
      mongodb.connect(this.url2).then(db => {
        ...
        stream
          ...
          .on('error', err => {
            this.db = null
            db.close()
            fs.appendFileSync('logs/error.log', `${new Date()}: Tailer.tail -> ${err} \n`)
            reject(err)
          })
      }))
  }
